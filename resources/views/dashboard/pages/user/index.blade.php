<x-app-layout>
  <x-slot name="header">
    <div class="w-full flex justify-between items-center">
      ...
      <a class="h-max w-max py-1 px-3 font-semibold text-white bg-blue-700 rounded"
        href="/dashboard/article/create">Nouvel article</a>
    </div>
  </x-slot>
                ...
                <tbody>
                  @foreach ($users as $user)
                    <tr class="hover:bg-grey-lighter">
                      ...
                      <td class="py-4 px-6 border-b border-grey-light">{{ $user->roles->name }}</td>
                      <td class="py-4 px-6 border-b border-grey-light">
                        ...
                        <form class="" action=" /dashboard/user/{{ $user->id }}" method="post">
                          @csrf
                          @method('DELETE')
                          <input
                            class="text-grey-lighter w-max font-bold py-1 px-3 rounded text-xs bg-red-600 text-white"
                            type="submit" value="X">
                        </form>
                      </td>
                    </tr>
